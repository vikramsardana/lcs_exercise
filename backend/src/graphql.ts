 ...
 */
function getCommitteeMembers(memberList: Member[], code: string): Member[] {
 ...
 */
function convertToMemberType(data: any): Member{
...
const resolvers = {
  Query: {
    ...
    committee: async(parent, args, contextValue, info) => {
      let result: Member[] = await getMembers();
